...
/**
 `BDCollectionView`数据源,代替`UICollectionViewDataSource`协议.使用注意如下:
 
 ①.`BDCollectionView`及其子类最好不要使用系统的`UICollectionViewDataSource`协议方法,否则在 dataSource 不是 self 的情况下数据展示会不正常(内部的 dataSource 设置始终是 self).
 ②.DataDource 需要实现`bdCollectionView:numberOfItemsInSection:`与`bdCollectionView:cellForItemAtIndexPath:`方法提供数据,此时系统的数据源协议要用此协议方法替换.
 */
...
/**
 BDScrollView 混合模式下,如果当前的 item 是`UICollectionView`的话,而且在默认支持自适应 contenSize 的条件下,该 item 需要是`BDCollectionView`的实例或其子类的实例.
 
 对于layout,现在十分方便，已经完成了普通 CollectionView 的布局还有瀑布流支持,只需要调取`bd_collectionViewLayout`属性获得`BDCollectionViewFlowLayout`的实例,简单设置列数等参数,在不设置`itemHeight`的添加下通过代理提供item的高就可以了.
 */
@interface BDCollectionView : UICollectionView

/**
 *  设置 collection view 的数据源与代理,遵守`BDCollectionViewDataSource` & `UICollectionViewDelegate`协议.
 */
...
/**
 *  collection view 的 header view,在`BDScrollView`支持下拉刷新的模式下,用于顶部留白的占位.
 */
@property (nonatomic, readonly, strong) UIView * collectionHeadView;

/**
 *  继承自`UICollectionViewFlowLayout`的`BDCollectionViewFlowLayout`,是当前 collection view 的 flowlayout.用户可以通过这个属性拿到 collection view 的 flowlayout, 设置一些参数进行简单布局.
 */
...
/**
 `BDCollectionViewFlowLayout`数据源,可以实现`BDCollectionView:layout:heightForItemAtIndexPath:`来动态设置 item 的高度.
 */
...
/**
 `BDCollectionView`专用的 flowlayout,用户需要获取`BDCollectionView`的`BDCollectionViewFlowLayout`来设置参数进行布局.
 
 现功能可以支持普通 collection view 布局 & 瀑布流布局.
 用户自定义 flowlayout 需要继承自`BDCollectionViewFlowLayout`,并且要在重写`prepareLayout`,`layoutAttributesForElementsInRect`与`collectionViewContentSize`方法的时候调用父类的方法.
 例如:
 - (void)prepareLayout {
 [super prepareLayout];
 ...
 // fit mincontentSize
 BDCollectionView * collectionView = (BDCollectionView *)self.collectionView;
...
/**
 *  需要自定义 flowlayout 的时候,自定义的 flowlayout 需要继承自`BDCollectionViewFlowLayout`,并且在布局 Attributes 的时候,第一个 Attributes 的 orgin.y 需要从`topOffsetOfFirstAttributes`开始,而不是0点的位置
 */
